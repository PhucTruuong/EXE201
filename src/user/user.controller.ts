...
export class UserController {
  ...
  async updateUser(@Body() user: UserModifiedDto) {
    ...
    if (
      userUpdated instanceof InternalServerErrorException ||
      NotFoundException
    ) {
  ...
  async disableUserAccount(@Param('id') id: string) {
    ...
    if (
      disableAccount instanceof InternalServerErrorException ||
      NotFoundException ||
      NotImplementedException
    ) {
      return disableAccount as
        | InternalServerErrorException
        | NotFoundException
        | NotImplementedException;
    }
  ...
  @ApiResponse({
    status: 200,
    description: '[User] It will get profile   in the response',
  })
