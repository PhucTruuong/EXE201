...
export class PetBreedRepository implements IPetBreed {
    ...
    async findAllPetBreed(pagination: PetBreedPagination): Promise<InternalServerErrorException | HttpException | { data: object[]; totalCount: number; }> {
        try {
            const { count, rows: allPetBreed } = await this.petBreedModel.findAndCountAll({
                attributes: [
                    'id',
                    'breed_name',
                    'breed_description',
            ...
            console.log(error);
            throw new InternalServerErrorException("Error fetching pet  breed", error)
        };
    }

    async findOnePetBreed(id: string): Promise<object | InternalServerErrorException | HttpException> {
        try {
            ...
            console.log(error);
            throw new InternalServerErrorException("Error find one  pet breed", error)
        };
    }
    async updatePetBreed(id: string, updatePetType: UpdatePetBreedDto): Promise<object | InternalServerErrorException | HttpException> {
        try {
            const petBreed = await this.petBreedModel.findOne({
                where: { id: id }
            })
    ...
    async deletePetBreed(id: string): Promise<object | InternalServerErrorException | HttpException> {
        try {
            ...
            console.log(error);
            throw new InternalServerErrorException("Error delete one pet breed", error)
        };
    }
}
